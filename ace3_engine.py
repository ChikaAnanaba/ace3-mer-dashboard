...
class ACE3Engine:
    ...
    def compute(self, quarter='Q1'):
        ...
        # ── PVLS Pregnant + BF + Post Partum disaggregation ──────────────
        # Eligible = TX_CURR with Pregnancy Status in [Pregnant, Breastfeeding, Post Partum]
        # D = Eligible with VL in window | N = D and suppressed
        _preg_status = txc['Pregnancy Status'].astype(str).str.strip()
        _preg_cats   = ['Pregnant', 'Breastfeeding', 'Post Partum']
        _preg_pids   = set(txc[_preg_status.isin(_preg_cats)]['Patient ID']
                           .astype(str).str.strip().str.upper())
        pvd['_pid_up'] = pvd['Patient ID'].astype(str).str.strip().str.upper()
        pvn['_pid_up'] = pvn['Patient ID'].astype(str).str.strip().str.upper()
        pvls_preg_elig = len(_preg_pids)
        pvls_preg_d    = int(pvd['_pid_up'].isin(_preg_pids).sum())
        pvls_preg_n    = int(pvn['_pid_up'].isin(_preg_pids).sum())

        # Per-category breakdown
        pvls_by_preg = {}
        for _cat in _preg_cats:
            _cat_pids = set(txc[_preg_status == _cat]['Patient ID']
                            .astype(str).str.strip().str.upper())
            pvls_by_preg[_cat] = {
                'tx_curr': len(_cat_pids),
                'd': int(pvd['_pid_up'].isin(_cat_pids).sum()),
                'n': int(pvn['_pid_up'].isin(_cat_pids).sum()),
            }
        ...
        r['TX_PVLS'] = {
            ...
            # Pregnant + BF + Post Partum disaggregation
            'preg_elig': pvls_preg_elig,
            'preg_d':    pvls_preg_d,
            'preg_n':    pvls_preg_n,
            'preg_by_cat': pvls_by_preg,
        }
        ...
        # ── TX_ML / TX_RTT ──
        # MER v2.8: patients who were on ART at the START of the reporting period
        # = Previous ART Status was Active or Active Restart (or blank = not recorded)
        # AND Client Verification is not Invalid
        sd    = self._valid['Date of Current ART Status']
        _prev = self._valid['Previous ART Status'].astype(str).str.strip()
        _cvo  = self._valid['Client Verification Outcome'].astype(str).str.strip().str.lower()
        non_active   = ['IIT', 'Died', 'Transferred Out', 'Stopped Treatment']
        invalid_cvos = ['invalid - duplicates', 'invalid - nonexistent',
                        'invalid - biometrical naive', 'invalid - transferred out']

        # Exact match to Power BI:
        # Previous ART Status = Active, Active Restart, or blank (nan/empty)
        # AND CVO = Valid or blank (not Invalid)
        _was_active = (
            _prev.isin(['Active', 'Active Restart', 'nan', 'None', ''])
        ) & (
            _cvo.isin(['valid', 'nan', 'none', ''])
        )

        ml_base = self._valid[
            self._valid['Current ART Status'].isin(non_active) & _was_active
        ]

        ml_fy  = ml_base[(sd >= FY_START)  & (sd <= FY_END)]
        ml_q1  = ml_base[(sd >= Q1_START)  & (sd <= Q1_END)]
        ml_q2  = ml_base[(sd >= Q2_START)  & (sd <= Q2_END)]
        ml_q3  = ml_base[(sd >= Q3_START)  & (sd <= Q3_END)]
        ml_q4  = ml_base[(sd >= Q4_START)  & (sd <= Q4_END)]
        ml_cum = ml_base[(sd >= FY_START)  & (sd <= Q2_END)]
        ...
        if self.tb is not None:
            ...
            # ── TB_PREV (D and N) — exact TPT_Start_Flag DAX ──────────────────
            # PID_N = UPPER(TRIM(Patient ID)) — calculated column in PBI
            # TPT_Start_Flag = 1 IF:
            #   PID exists as Active OR Active Restart in FY26 RADET
            #   AND TPT Type is not blank
            # TB_PREV_D = DISTINCTCOUNT(PID_N) where TPT_Start_Flag = 1
            # TB_PREV_N = from D, those with TPT completion status = completed
            # NO date filter — this is programme total (confirmed DAX)
            active_pids = set(
                self._valid[self._valid['Current ART Status'].isin(ACTIVE)]['Patient ID']
                .astype(str).str.strip().str.upper()
            )
            self.tb['_pid_n'] = self.tb['Patient ID'].astype(str).str.strip().str.upper()
            self.tb['_tpt_type_c'] = self.tb['TPT Type'].astype(str).str.strip()
            self.tb['_tpt_comp_c'] = self.tb['TPT completion status'].astype(str).str.strip().str.lower()

            tb_tpt_flag = self.tb[
                (self.tb['_pid_n'] != '') &
                (self.tb['_pid_n'] != 'NAN') &
                self.tb['_pid_n'].isin(active_pids) &
                self.tb['_tpt_type_c'].notna() &
                (~self.tb['_tpt_type_c'].isin(['', 'nan', 'None']))
            ]

            tb_prev_d_pids = tb_tpt_flag['_pid_n'].unique()
            tb_prev_d      = len(tb_prev_d_pids)

            comp_ok = ['treatment completed', 'ipt completed', 'treatment complete']
            tb_prev_n = tb_tpt_flag[tb_tpt_flag['_tpt_comp_c'].isin(comp_ok)]['_pid_n'].nunique()

            tpt_coverage = round(tb_prev_d / len(txc) * 100, 1) if len(txc) > 0 else 0

            # TB_SCREEN also populates TX_TB_D — use screened count
            # TX_TB_N = currently on TB treatment (from RADET)
            tx_tb_n = int((txc['TB status'].astype(str).str.strip() == 'Currently on TB treatment').sum())

            r['TPT'] = {
                'started_tb':      tb_prev_d,
            ...
            r['TX_TB'] = {
                # TX_TB_D is computed manually from programme data — placeholder 0
                # Override this value in the app before generating reports
                'D':        0,
                'N':        tx_tb_n,
                'placeholder': True,
            }
