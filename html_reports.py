...
def _build_alerts(d, q1=None, q2=None):
    ...
    if cxca_e and cxca_t:
        cov=((cxca_t/cxca_e*100 if cxca_e else 0) if cxca_e else 0)
    ...
    if deliv and eid and (eid/deliv*100 if deliv else 0)<95: alerts.append(f"EID PCR {((eid/deliv*100 if deliv else 0) if deliv else 0):.1f}% of deliveries")
    return alerts
...
def build_dashboard(results, period, quarter_label, quarter_mode):
    ...
    eid_c=(eid/deliv*100 if deliv else 0) if deliv else 0
    tpt_c=tpt/txc*100 if txc else 0
    cxca_c=(cxca_t/cxca_e*100 if cxca_e else 0) if cxca_e else 0
    ahd_r=((ahd_c/ahd_s*100 if ahd_s else 0) if ahd_s else 0) if ahd_s else 0
...
<div class="kpi-row">
  ...
  <div class="kpi navy">
    ...
    <div class="kpi-sub">{fem_p:.1f}% female · {(((lt15/txc*100 if txc else 0) if txc else 0) if txc else 0):.1f}% paediatric</div>
  ...
  <div class="kpi teal">
    ...
    <div class="kpi-sub">{_n(pmtc_p)} HIV+ · {(((pmtc_p/pmtc_n*100 if pmtc_n else 0) if pmtc_n else 0) if pmtc_n else 0):.3f}% prev.</div>
  ...
  <div class="card">
    ...
    <div class="mini-3">
      <div class="mt" style="background:var(--sblue);border-color:#C0D8EE">
        ...
        <div class="mt-l">HIV+ at ANC<br>{((pmtc_p/pmtc_n*100 if pmtc_n else 0) if pmtc_n else 0):.3f}%</div>
      </div>
      <div class="mt" style="background:var(--sblue);border-color:#C0D8EE">
...
<div class="g32">
  <div class="card">
    ...
    <div class="cs">Total {_n(txc)} · {fem_p:.1f}% female · {((lt15/txc*100 if txc else 0) if txc else 0):.1f}% paediatric · {bio_p:.1f}% biometric</div>
  ...
  <div class="card">
    ...
    <div style="display:flex;gap:12px;align-items:center;margin-top:4px">
      ...
      <div class="leg">
        <div class="li"><div class="ld" style="background:var(--green)"></div><span class="lt">6+ months</span><span class="lv">{_n(mmd_6p)}</span><span class="lp">{(((mmd_6p/txc*100 if txc else 0) if txc else 0) if txc else 0):.1f}%</span></div>
        <div class="li"><div class="ld" style="background:var(--blue)"></div><span class="lt">3–5 months</span><span class="lv">{_n(mmd_35)}</span><span class="lp">{(((mmd_35/txc*100 if txc else 0) if txc else 0) if txc else 0):.1f}%</span></div>
        <div class="li"><div class="ld" style="background:var(--red)"></div><span class="lt">&lt;3 months</span><span class="lv">{_n(mmd_lt3)}</span><span class="lp">{(((mmd_lt3/txc*100 if txc else 0) if txc else 0) if txc else 0):.1f}%</span></div>
      </div>
    </div>
...
<div class="g32">
  ...
  <div class="card">
    ...
    <div class="ahd-g">
      <div class="ahd-t"><div class="ahd-v" style="color:var(--navy)">{_n(ahd_cd4)}</div><div class="ahd-l">CD4 Tests Done</div><div class="ahd-s">{(((ahd_cd4/ahd_s*100 if ahd_s else 0) if ahd_s else 0) if ahd_s else 0):.1f}% of screened</div></div>
...
def build_narrative(results, period, quarter_label, quarter_mode):
    ...
    ahd_r=(ahd_c/ahd_s*100 if ahd_s else 0) if ahd_s else 0
    fem_p=fem/txc*100 if txc else 0
    bio_p=bio/txc*100 if txc else 0
    link_r=tx_new/hts_p*100 if hts_p else 0
...
<div class="report-wrap">
  ...
  <div class="report-section">
    ...
    <p class="report-p"><strong>{_n(pmtc_n)} pregnant women were tested for HIV at ANC</strong> during {period}, identifying <strong>{_n(pmtc_p)} HIV-positive women</strong> (prevalence {((pmtc_p/pmtc_n*100 if pmtc_n else 0) if pmtc_n else 0):.3f}%).
  ...
  <div class="report-section">
    ...
    <p class="report-p">As of {period}, <strong>{_n(txc)} clients are active on ART</strong> across 37 facilities ({fem_p:.1f}% female, {((lt15/txc*100 if txc else 0) if txc else 0):.1f}% paediatric, {bio_p:.1f}% biometrically enrolled).
    <strong>{_p(mmd_p)}</strong> of clients ({_n(mmd_3p)}) are on multi-month dispensing of ≥3 months, with {_p(((mmd_6p/txc*100 if txc else 0) if txc else 0) if txc else 0)} on a 6-month supply — {'exceeding' if mmd_p>=90 else 'approaching'} the 90% DSD threshold.</p>
...
def build_talking_points(results, period, quarter_label, quarter_mode):
    ...
    cxca_e=d.get('CXCA_SCRN',0); cxca_t=d.get('CXCA_TX',0); cxca_c=(cxca_t/cxca_e*100 if cxca_e else 0) if cxca_e else 0
    eid=d.get('PMTCT_EID',0); deliv=d.get('PMTCT_DELIVERED',0); eid_c=(eid/deliv*100 if deliv else 0) if deliv else 0
